...
\begin{frame}{Pseudocode for finding eigenvalues and eigenvectors}
    ...
    \begin{algorithmic}
        ...
        \EndWhile
    \end{algorithmic}
    The function implementing the Hessemberg reduction must return the matrix used for the transformation, meaning that $B = P H P^T$. \newline
    $P$ is updated in this way because $Q_i = G_{12}^i G_{23}^i \cdots G_{n-1 n}^i$, where $i$ denotes the generic iteration for error minimization.
\end{frame}
